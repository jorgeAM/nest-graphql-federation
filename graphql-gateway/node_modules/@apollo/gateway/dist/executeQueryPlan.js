...
async function executeQueryPlan(queryPlan, serviceMap, requestContext, operationContext, supergraphSchema) {
...
async function executeNode(context, node, results, path, captureTraces) {
    ...
    switch (node.kind) {
        ...
        case 'Condition': {
            (0, federation_internals_1.assert)(false, `Condition nodes are not available in the gateway`);
        }
    }
}
async function executeFetch(context, fetch, results, _path, traceNode) {
    ...
    return opentelemetry_1.tracer.startActiveSpan(opentelemetry_1.OpenTelemetrySpanNames.FETCH, { attributes: { service: fetch.serviceName } }, async (span) => {
        try {
            ...
            if (!fetch.requires) {
                const dataReceivedFromService = await sendOperation(context, fetch.operation, variables, fetch.operationName, fetch.operationDocumentNode);
                ...
                const dataReceivedFromService = await sendOperation(context, fetch.operation, { ...variables, representations }, fetch.operationName, fetch.operationDocumentNode);
    ...
    async function sendOperation(context, source, variables, operationName, operationDocumentNode) {
        var _a, _b;
        let http;
        if (traceNode) {
            http = {
...
function downstreamServiceError(originalError, serviceName) {
    ...
    let codeDef = (0, federation_internals_1.errorCodeDef)(originalError);
