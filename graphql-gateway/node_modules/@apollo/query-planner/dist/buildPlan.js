...
class QueryPlanningTaversal {
    constructor(supergraphSchema, subgraphs, selectionSet, startFetchIdGen, hasDefers, variableDefinitions, startVertex, rootKind, costFunction, initialContext, excludedEdges = [], excludedConditions = []) {
    ...
    resolveConditionPlan(edge, context, excludedEdges, excludedConditions) {
        const bestPlan = new QueryPlanningTaversal(this.supergraphSchema, this.subgraphs, edge.conditions, 0, false, this.variableDefinitions, edge.head, 'query', this.costFunction, context, excludedEdges, (0, query_graphs_1.addConditionExclusion)(excludedConditions, edge.conditions)).findBestPlan();
...
class FetchGroup {
    ...
    addParent(parent) {
        (0, federation_internals_1.assert)(!this.isChildOf(parent.group), () => `Group ${parent.group} is already a parent of ${this}`);
        (0, federation_internals_1.assert)(!parent.group.isParentOf(this), () => `Group ${parent.group} is already a parent of ${this} (but the child relationship is broken)`);
    ...
    mergeInInternal(merged, path, mergeParentDependencies = false) {
        ...
        if (path.length === 0) {
            ...
            selectionSet = (0, federation_internals_1.selectionSetOfPath)(path, (endOfPathSet) => {
...
function computePlanInternal({ supergraphSchema, federatedQueryGraph, operation, processor, root, hasDefers, }) {
...
function computePlanForDeferConditionals({ supergraphSchema, federatedQueryGraph, operation, processor, root, deferConditions, }) {
...
function computeRootParallelDependencyGraph(supergraphSchema, operation, federatedQueryGraph, root, startFetchIdGen, hasDefer) {
    return computeRootParallelBestPlan(supergraphSchema, operation.selectionSet, operation.variableDefinitions, federatedQueryGraph, root, startFetchIdGen, hasDefer)[0];
}
function computeRootParallelBestPlan(supergraphSchema, selection, variables, federatedQueryGraph, root, startFetchIdGen, hasDefers) {
    const planningTraversal = new QueryPlanningTaversal(supergraphSchema, federatedQueryGraph, selection, startFetchIdGen, hasDefers, variables, root, root.rootKind, defaultCostFunction, query_graphs_1.emptyContext);
...
function computeRootSerialDependencyGraph(supergraphSchema, operation, federatedQueryGraph, root, hasDefers) {
...
function addPostRequireInputs(dependencyGraph, entityType, edge, context, preRequireGroup, requirePath, postRequireGroup) {
    ...
    if (keyInputs) {
        const addedSelection = (0, federation_internals_1.selectionSetOfPath)(requirePath, (endOfPathSet) => {
